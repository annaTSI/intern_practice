...
--   order_items → 37 rows
	...
	select avg (salary),min(salary),max(salary)from employees where hire_date between '2022-01-01' and '2022-12-31'
	select customer_id,count(order_id) as total_orders, sum(amount) as total_spend from orders group by customer_id having count(order_id) >=3
	select name,department,salary, dept_avg from ( select name, department, salary,avg(salary) over(partition by department) as dept_avg from employees) as employee_stats where salary > dept_avg;
	select name, price, case when price >1000 then 'expensive' when price between 500 and 1000 then 'moderate' else 'afforadable' end as category_label from products;
    select  o.order_id, c.name as customer_name, o.order_date,  datediff(curdate(), o.order_date) as days_since_order from orders o join customers c on o.customer_id = c.customer_id where o.order_date >= curdate() - interval 90 day order by o.order_date desc;
	select name,department,salary, rank() over (partition by department order by salary desc ) as salary_rank from employees
	with productrevenue as ( select  p.category, p.name as product_name,sum(oi.quantity * oi.unit_price) as total_revenue from  products p join  order_items oi on p.product_id = oi.product_id group by  p.category, p.name ),rankedproducts as ( select  category, product_name, total_revenue, dense_rank() over (partition by category order by total_revenue desc) as revenue_rank from  productrevenue)select  category, product_name, total_revenue from  rankedproducts where  revenue_rank <= 3;
	select *from employees w, employees m where w.manager_id = m.emp_id and w.salary> m.salary;
	with cte as (select customer_id, order_date, lag(order_date,1) over(partition by customer_id order by order_date) as prev_order_date from orders) select * from cte where datediff(order_date,prev_order_date)=1;
